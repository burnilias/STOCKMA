...
<template>
  <div class="mx-auto max-w-lg py-6">
    <form class="card space-y-6 !p-8" @submit.prevent="submit">
      <div class="space-y-1">
        <h2 class="text-xl font-black tracking-tight text-slate-900">Sortie de stock</h2>
        <p class="text-xs font-bold text-slate-400">Enregistrer une sortie de marchandises.</p>
      </div>
      
      <div class="space-y-4">
        <div class="space-y-1.5">
          <label class="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">Produit</label>
        ...
        <div class="space-y-1.5">
          <label class="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">Motif</label>
          <input v-model="form.reason" class="input-field !py-2.5 !text-xs" placeholder="Ex. Vente, Perte, Usage…" />
        </div>

        <div class="space-y-1.5">
          <label class="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">Note</label>
          <textarea v-model="form.note" rows="2" class="input-field resize-none !py-2.5 !text-xs" placeholder="Détails facultatifs..." />
        </div>
      </div>
      ...
      <button type="submit" :disabled="loading" class="btn-primary w-full !py-3.5 !text-sm shadow-xl shadow-brand-600/20">
        ...
        <template v-else>Confirmer la sortie</template>
      </button>
    </form>
  </div>
</template>
